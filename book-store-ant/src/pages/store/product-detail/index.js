...
const ProductDetail = () => {
  ...
  const [productBoughtTogether, setProductBoughtTogether] = useState([]);
  ...
  const { recommendationsTogether } = useFrequentlyBoughtTogether({
    recommendClient,
    indexName,
    objectIDs: [id],
  });
  ...
  useEffect(() => {
    if (recommendations?.length > 0) {
      ...
      recommendations.forEach(
        (item, index) => index <= 4 && newList.push(item)
      );
      setProductFeatures(newList);
    }
  }, [recommendations]);

  useEffect(() => {
    if (recommendationsTogether?.length > 0) {
      const newList = [];
      recommendationsTogether.forEach(
        (item, index) => index <= 4 && newList.push(item)
      );
      setProductBoughtTogether(newList);
    }
  }, [recommendationsTogether]);
  ...
  return productDetail.status === false ? (
    ...
    <>
                ...
                <Col span={12} style={{ textAlign: 'center' }}>
                  <Form.Item style={{ width: '100%', textAlign: 'center' }}>
                    <button
                      ...
                      className="btn-cart"
                      onClick={() => {
                        requestRecommendationsConverted(productDetail._id);
                        addToCart(false);
                      }}
                    >
      ...
      <WrapperConentContainer className="products-related">
              ...
              <Col flex={'19%'} style={{ marginBottom: '30px' }}>
                <Card
                  ...
                  cover={
                    <img
                      onClick={() => {
                        requestRecommendationsClick(productDetail._id);
      ...
      {productBoughtTogether?.length > 0 && (
        <WrapperConentContainer className="products-frequently-bought-together">
          <Col className="products-content">
            <h2
              style={{
                display: 'flex',
            ...
            >
              ...
              />
              Mọi người thường mua cùng
            </h2>
            <Divider style={{ margin: '18px 0' }} />
            <Row justify="space-evenly">
              {productBoughtTogether?.map((item) => (
                <Col flex={'19%'} style={{ marginBottom: '30px' }}>
                  <Card
                    className="product-card"
                    hoverable={false}
